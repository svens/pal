...
TEST_CASE("net/ip/address_v6")
{
	...
	{
		auto v = make_address_v6(view.c_str());
		REQUIRE(v);
		CHECK(*v == a);
	}
	...
	{
		auto v = make_address_v6(invalid_view.c_str());
		REQUIRE_FALSE(v);
	...
	{
		auto v = make_address_v6(std::string_view{view});
		REQUIRE(v);
		CHECK(*v == a);
	}
	...
	{
		auto v = make_address_v6(std::string_view{invalid_view});
		REQUIRE_FALSE(v);
		CHECK(v.error() == std::errc::invalid_argument);
	}
