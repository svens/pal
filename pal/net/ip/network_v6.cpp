...
std::to_chars_result network_v6::to_chars (char *first, char *last) const noexcept
{
...
std::from_chars_result network_v6::from_chars (const char *first, const char *last) noexcept
{
	address_v6 addr;
	...
	address_ = address_v6{__network_v6::masked(addr.to_bytes(), prefix_length_)};
	return r;
}
