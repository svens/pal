...
TEMPLATE_TEST_CASE("net/ip/basic_endpoint", "", tcp, udp)
{
	...
	SECTION("constexpr")
	{
		...
		CHECK(e2.address() == A4::any());

		E e3{TestType::v6(), port};
	...
	auto &[view, protocol, address] = GENERATE(
		table<std::string, TestType, A>({
