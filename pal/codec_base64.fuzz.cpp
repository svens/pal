...
constexpr size_t max_encoded = pal::base64_encoder::max_size(max_input);
constexpr size_t max_decoded = pal::base64_decoder::max_size(max_input + max_input / 2);
...
extern "C" int LLVMFuzzerTestOneInput (const uint8_t *data, std::size_t size)
{
	...
	std::ignore = pal::convert(pal::base64_decode, decoded, blob);
	...
	auto [enc_ptr, enc_ec] = pal::convert(pal::base64_encode, encoded, blob);
	if (enc_ec != std::errc{})
	{
		return 0;
	}

	auto [dec_ptr, dec_ec] = pal::convert(pal::base64_decode, decoded, std::string_view{encoded.data(), enc_ptr});
	if (dec_ec != std::errc{})
	{
		std::abort();
	}
