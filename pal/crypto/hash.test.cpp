...
struct test_data<md5>
{
	...
	static inline const std::unordered_map<std::string_view, std::string_view> expected =
	{
		...
		{ lazy_dog_cog, "29b4e7d924350ff800471c80c9ca2a3f" },
	};
};
...
struct test_data<sha1>
{
	...
	static inline const std::unordered_map<std::string_view, std::string_view> expected =
	{
		...
		{ lazy_dog_cog, "38590c861cc71a4186b2909285a04609fb23bb42" },
	};
};
...
struct test_data<sha256>
{
	...
	static inline const std::unordered_map<std::string_view, std::string_view> expected =
	{
		...
		{ lazy_dog_cog, "0a9a361e469fd8fb48e915a06431f3fabbfb0960226421a25ab939fde121b7c8" },
	};
};
...
struct test_data<sha384>
{
	...
	static inline const std::unordered_map<std::string_view, std::string_view> expected =
	{
		...
		{ lazy_dog_cog, "03b251e870443c1dc8052967970cc91bdd3bd5c3784ea0b2df52f0f4a6c56f947fcc1369b593730479dc07d73a043297" },
	};
};
...
struct test_data<sha512>
{
	...
	static inline const std::unordered_map<std::string_view, std::string_view> expected =
	{
		...
		{ lazy_dog_cog, "9a1eacc4b2de80d412e8e28aa918c22450246c9d249559e6cba45145feebd05298c8d91cde493acd7c2bf9ed5c86612a7f8c8323c10913d8b4703c8d6bcd99f8" },
	};
};

// NOLINTEND(bugprone-throwing-static-initialization)

TEMPLATE_TEST_CASE("crypto/hash", "", md5, sha1, sha256, sha384, sha512)
{
	...
	SECTION("multiple updates")
	{
		...
		CHECK(to_hex(hash.finish()) == expected.at(lazy_dog_cog));
	}

	SECTION("move assign")
	{
		auto h1 = Hash::make().value();
		h1.update(std::span{lazy_dog});
		auto h2 = Hash::make().value();
		h2 = std::move(h1);
	...
	{
		CHECK(to_hex(Hash::one_shot(std::span{lazy_dog}, std::span{lazy_cog}).value()) == expected.at(lazy_dog_cog));
	}
	...
	{
		std::vector<std::byte> buf(Hash::digest_size - 1);
		const auto out = hash.finish(buf);
		REQUIRE(!out.has_value());
