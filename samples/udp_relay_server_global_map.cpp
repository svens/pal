...
private:
	...
	static inline std::unordered_map<client_id, endpoint_type> client_map_{};
	static inline std::shared_mutex client_map_mutex_{};
	...
	{
		std::unique_lock lock{client_map_mutex_};
		client_map_.emplace(id, endpoint);
	}

	static const endpoint_type *try_load (client_id id)
	{
		std::shared_lock lock{client_map_mutex_};
