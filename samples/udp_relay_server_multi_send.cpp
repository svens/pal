...
// udp_relay_server_multi_send variant
...
//   thread-specific local map (unprotected)
// * all per-threads sends are batched together and invoked as single syscall
//   if supported by OS (Linux sendmmsg)
...
public:
	...
	listener (const config &config, bool &stopped)
		...
		, send_batch_{client_.async_send_many()}
	{
...
private:
	...
	pal::net::async::multi_send_request<protocol> send_batch_;
	metrics metrics_{};
		...
		void on_peer_receive ()
		{
			...
			if (receive.bytes_transferred >= sizeof(client_id))
			{
				if (auto *client_endpoint = listener.try_load(get_client_id(data)))
				{
					...
					listener.send_batch_.push_back(
						&handle,
