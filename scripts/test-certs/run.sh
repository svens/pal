...
#
# Name/SAN fields in test.hpp (subject_name, issuer_name, etc.) are written
# as empty strings. After running the test suite once, copy the actual values
# printed by failing tests into those fields.
#
...
{
	echo "// Generated by scripts/test-certs/run.sh -- do not edit"
...

{
	echo "// Generated by scripts/test-certs/run.sh -- do not edit"
	echo "// clang-format off"
	echo "// NOTE: subject_name, subject_alternative_name, issuer_name, issuer_alternative_name"
	echo "// are intentionally empty -- fill them in after implementing distinguished_name"
	echo "// and alternative_name (Cluster B steps 3-4) from failing test output."
	echo "// Remove this note generation from run.sh once those are implemented: the script's"
	echo "// 'Next steps' output then closes the loop and this warning is no longer needed."
	echo "#pragma once"
	echo ""
	echo "namespace pal_test::cert"
	echo "{"
...
echo ""
echo "Next steps:"
echo "  1. Build and run the certificate tests"
echo "  2. Copy the actual subject_name / issuer_name / *_alternative_name"
echo "     values from failing test output into $CERTS_OUT"
